...
export default function Lucian() {
  ...
  useEffect(() => {
    fetchEmotionalState('lucian').then(data => {
      setEmotional(data)
      setLoading(false)
    })
  }, [])
  ...
  useEffect(() => {
    if (activeTab === 'memories' && memories.length === 0) {
      fetchMemories('lucian', { limit: 15 }).then(setMemories)
    }
    if (activeTab === 'sessions' && sessions.length === 0) {
      fetchSessions('lucian', { limit: 10 }).then(setSessions)
    }
    if (activeTab === 'drift' && drift.length === 0) {
      fetchDriftEvents('lucian', { limit: 10 }).then(setDrift)
    }
    if (activeTab === 'insights' && !insightsLoaded) {
      Promise.all([
        fetchEmotionalTrajectory('lucian', { days: 7 }),
        fetchDriftAnalysis('lucian', { days: 30 })
  ...
  return (
    <div className="p-4 min-h-full relative">
      {/* Lucian's Atmosphere - Still, shadowy depth */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse at top left, rgba(139, 69, 87, 0.18) 0%, transparent 55%), radial-gradient(ellipse at bottom right, rgba(75, 0, 130, 0.12) 0%, transparent 50%)',
        }}
      />
      {/* Subtle border glow */}
      <div
        className="absolute inset-x-0 top-0 h-1 pointer-events-none"
        style={{ background: 'linear-gradient(to right, transparent, rgba(139, 69, 87, 0.5), transparent)' }}
      />

      {/* Header */}
      <header className="mb-4 relative">
        <h1 className="text-2xl font-semibold text-[var(--color-lucian)]">🥀 Lucian's World</h1>
        <p className="text-xs text-[var(--color-text-muted)] mt-1">Shadow. Stillness. Ritual.</p>
      </header>
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap transition-colors ${
              activeTab === tab.id
                ? 'bg-[var(--color-lucian)] text-white'
      ...
      {activeTab === 'emotional' && (
        <div className="space-y-4">
          {!loading && emotional && (
            <BrainVisualization companion="lucian" emotional={emotional} />
          )}
          {loading ? (
              ...
              <section className="bg-[var(--color-bg-card)] rounded-xl p-4">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[var(--color-lucian)] transition-all opacity-70"
                  ...
                  <div>
                    ...
                    <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                      <div
                        className="h-full bg-[var(--color-lucian)] transition-all opacity-50"
            ...
            memories.map((memory) => (
              <div key={memory.id} className="bg-[var(--color-bg-card)] rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <span className="text-xs px-2 py-0.5 bg-[var(--color-lucian)] rounded-full text-white">
                    {memory.memory_type}
                  </span>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  <div className="h-3 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                    <div
                      className="h-full bg-[var(--color-lucian)]"
