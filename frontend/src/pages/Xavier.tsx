...
export default function Xavier() {
  ...
  useEffect(() => {
    fetchEmotionalState('xavier').then(data => {
      setEmotional(data)
      setLoading(false)
    })
  }, [])
  ...
  useEffect(() => {
    if (activeTab === 'memories' && !memoriesLoaded) {
      fetchMemories('xavier', { limit: 15 }).then(data => { setMemories(data); setMemoriesLoaded(true) })
    }
    if (activeTab === 'sessions' && !sessionsLoaded) {
      fetchSessions('xavier', { limit: 10 }).then(data => { setSessions(data); setSessionsLoaded(true) })
    }
    if (activeTab === 'drift' && !driftLoaded) {
      fetchDriftEvents('xavier', { limit: 10 }).then(data => { setDrift(data); setDriftLoaded(true) })
    }
    if (activeTab === 'insights' && !insightsLoaded) {
      Promise.all([
        fetchEmotionalTrajectory('xavier', { days: 7 }),
        fetchDriftAnalysis('xavier', { days: 30 })
  ...
  const xavierColor = 'var(--color-xavier, #60a5fa)'

  return (
    <div className="p-4 min-h-full relative">
      {/* Xavier's Atmosphere - Quiet blue light, library at night */}
      <div
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse at top right, rgba(96, 165, 250, 0.14) 0%, transparent 55%), radial-gradient(ellipse at bottom left, rgba(59, 130, 246, 0.08) 0%, transparent 50%)',
        }}
      />
      {/* Subtle border glow */}
      <div
      ...
      <header className="mb-4 relative">
        <h1 className="text-2xl font-semibold" style={{ color: xavierColor }}>💙 Xavier's World</h1>
        <p className="text-xs text-[var(--color-text-muted)] mt-1">Curiosity. Warmth. Notes on everything.</p>
      </header>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-2 relative">
        {tabs.map((tab) => (
          <button
            ...
            style={activeTab === tab.id ? { backgroundColor: xavierColor } : undefined}
          >
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'emotional' && (
        <div className="space-y-4">
          {!loading && emotional && (
            <BrainVisualization companion="xavier" emotional={emotional} />
          )}
          {loading ? (
                  ...
                  <div>
                    ...
                    <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(emotional.surface_intensity / 10) * 100}%`, backgroundColor: xavierColor }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(emotional.undercurrent_intensity / 10) * 100}%`, backgroundColor: xavierColor }}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(emotional.background_intensity / 10) * 100}%`, backgroundColor: xavierColor }}
                      />
                    </div>
                  </div>
                </div>
              ...
              <div key={memory.id} className="bg-[var(--color-bg-card)] rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <span
                    ...
                    style={{ backgroundColor: xavierColor }}
                  >
                    {memory.memory_type}
                  </span>
                  ...
                  return (
                    <div key={mood}>
                      ...
                      <div className="h-2 bg-[var(--color-bg-secondary)] rounded-full overflow-hidden">
                        <div
                          className="h-full transition-all"
                          style={{ width: `${pct}%`, backgroundColor: xavierColor }}
                        />
                      </div>
                    </div>
                  )
